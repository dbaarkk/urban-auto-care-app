...
export default function HomePage() {
  const { user, isLoading, updateLocation } = useAuth();
  ...
  const handleLocationSelect = async (address: string, coords: { lat: number; lng: number }) => {
    const result = await updateLocation(address, coords);
    if (result.success) {
    ...
    <div className="mobile-container bg-gray-50 min-h-screen safe-bottom">
      ...
      {showPicker && (
        <LocationPicker 
          ...
          initialAddress={user.locationAddress}
